    ...
    fn test_smartphone_misc() {
        ...
        match parser.parse(r#"Mozilla/5.0 (Mobile; rv:18.0) Gecko/18.0 Firefox/18.0"#) {
            ...
            Some(result) => {
                ...
                assert_eq!(result.os, "Firefox OS".to_string());
                // NOTE: skip test now
                // assert_eq!(result.os_version, "UNKNOWN".to_string());
                assert_eq!(result.version, "18.0".to_string());
            }
        }
        match parser.parse(r#"Mozilla/5.0 (Tablet; rv:26.0) Gecko/18.0 Firefox/26.0"#) {
            ...
            Some(result) => {
                ...
                assert_eq!(result.version, "26.0".to_string());
            }
        }
        match parser.parse(r#"Mozilla/4.0 (compatible; MSIE 7.0; Windows Phone OS 7.5; Trident/3.1; IEMobile/7.0; FujitsuToshibaMobileCommun; IS12T; KDDI)"#) {
            ...
            Some(result) => {
                ...
                assert_eq!(result.os, "Windows Phone OS".to_string());
        ...
        match parser.parse(r#"Mozilla/4.0 (compatible; MSIE 6.0; Windows CE; IEMobile 7.7) S12HT"#) {
            ...
            Some(result) => {
                assert_eq!(result.category, "smartphone".to_string());
                assert_eq!(result.name, "Internet Explorer".to_string());
                assert_eq!(result.os, "Windows CE".to_string());
                // NOTE: skip test now
                //assert_eq!(result.os_version, "UNKNOWN".to_string());
        ...
        match parser.parse(r#"Opera/9.80 (BlackBerry; Opera Mini/6.5.27548/26.1305; U; ja) Presto/2.8.119 Version/10.54"#) {
            ...
            Some(result) => {
                ...
                assert_eq!(result.os, "BlackBerry".to_string());
                assert_eq!(result.os_version, "UNKNOWN".to_string());
        ...
        match parser.parse(r#"Mozilla/5.0 (BB10; Touch) AppleWebKit/537.35+ (KHTML, like Gecko) Version/10.3.1.2243 Mobile Safari/537.35+"#) {
            ...
            Some(result) => {
                ...
                assert_eq!(result.os, "BlackBerry 10".to_string());
